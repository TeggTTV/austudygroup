...
export function Slider({ label, value, className = '', id, ...props }: SliderProps) {
  ...
  return (
    <div className="space-y-1.5 w-full">
      ...
      >
        ...
        <div className="absolute left-0 right-0 h-1.5 bg-border rounded-full flex justify-between px-[7px] pointer-events-none">
          {Array.from({ length: steps + 1 }).map((_, index) => (
            <div 
              key={index} 
              className={`w-[2px] h-[6px] self-center rounded-full transition-colors ${
                index + minVal <= value 
                  ? 'bg-primary/70' 
                  : 'bg-text-muted/30'
              }`}
