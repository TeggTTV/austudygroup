...
export default function PendingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'requests' | 'sent' | 'history'>('requests');
    ...
    return (
      <div className="flex min-h-screen flex-col bg-background">
        ...
        <main className="flex-grow flex items-center justify-center">
          <p className="text-text-muted text-sm">Please sign in to view requests.</p>
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
        <h1 className="text-2xl font-bold text-text-primary mb-1">Join Requests</h1>
                ...
                >
                  <div className="flex items-start space-x-4">
                    <img
                      ...
                      src={getUserAvatar(req.userId)}
                      alt={getUserName(req.userId)}
                    />
                    <div>
                      <h4 className="font-semibold text-text-primary">{getUserName(req.userId)}</h4>
                      <p className="text-xs text-text-muted">{getUserEmail(req.userId)}</p>
                      <p className="mt-1 text-xs text-text-secondary">
                        Wants to join <span className="font-medium text-primary">{getGroupName(req.groupId)}</span>
                      </p>
                    </div>
                  </div>
          ...
          sentPending.length === 0 ? (
            ...
            <div className="space-y-3">
              {sentPending.map((req) => (
                ...
                >
                  <div>
                    <h4 className="font-semibold text-text-primary">{getGroupName(req.groupId)}</h4>
                    <p className="text-xs text-text-muted mt-0.5">
                      Requested {new Date(req.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold border ${statusBadge(req.status)}`}>
                    {req.status}
                  </span>
                </div>
              ))}
          ...
          historyRequests.length === 0 ? (
            ...
            <div className="space-y-3">
              {historyRequests.map((req) => {
                const isSentByMe = req.userId === currentUser.id;
                return (
                  <div
                    key={req.id}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-text-muted mt-1">
                        {isSentByMe ? 'You requested to join' : `${getUserName(req.userId)} requested to join`}
                      </p>
                    </div>
                    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold border ${statusBadge(req.status)}`}>
                      {req.status}
